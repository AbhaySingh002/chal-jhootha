      ...
      <AnimatePresence mode="wait">
        {declarationOpen ? (
          ...
          <motion.div key="actions" initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: reduceMotion ? 0 : 0.16 }} className="game-action-row grid grid-cols-3 gap-2">
            ...
            <button type="button" onClick={skip} className="brutal-btn flex min-w-0 items-center justify-center gap-1.5 bg-ink px-2 text-xs text-white sm:text-sm" aria-label="Skip" title="Skip"><SkipForward size={18} strokeWidth={2.5} /><span className="hidden sm:inline">Skip</span></button>
          </motion.div>
        )}
      </AnimatePresence>
    </>
