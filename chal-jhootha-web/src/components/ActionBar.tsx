...
export const ActionBar: React.FC<ActionBarProps> = ({ selectedCards, clearSelection, declarationOpen, onDeclarationOpenChange }) => {
  ...
  const { progress, isUrgent } = useTurnTimer(gameState?.turnDeadlineUnixMs, gameState?.turnDurationMs);
  ...
  const isSelfTopPlay = Boolean(gameState.topPlay && gameState.topPlay.playerId === playerId);
  const canCallout = Boolean(gameState.topPlay && !isSelfTopPlay);
  ...
  const timerBar = gameState.turnDeadlineUnixMs ? (
    <div className="mb-2 h-[2.5px] w-full overflow-hidden rounded-full bg-ink/15" role="progressbar" aria-label="Turn timer">
      <div
        className={clsx(
          "h-full rounded-full transition-colors duration-200",
          isUrgent ? "bg-evidence-red animate-pulse" : isMyTurn ? "bg-caution-yellow" : "bg-ink"
        )}
        style={{
  ...
  if (!isMyTurn) return (
    <>
      {finalCalloutNotice}
      {timerBar}
      <p className="border-2 border-ink bg-ink px-3 py-2 text-center font-mono text-xs font-bold uppercase tracking-[0.08em] text-paper">{activeSuspect}'s turn</p>
    </>
  );
      ...
      <AnimatePresence mode="wait">
        {declarationOpen ? (
          ...
          <motion.div key="actions" initial={reduceMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: reduceMotion ? 0 : 0.16 }} className="game-action-row grid grid-cols-3 gap-2">
            ...
            <button type="button" disabled={!canCallout} onClick={challenge} className={clsx('brutal-btn flex min-w-0 items-center justify-center gap-1.5 px-2 text-xs sm:text-sm', canCallout ? 'bg-evidence-red text-white' : 'bg-surface-muted text-ink-muted')} aria-label={isSelfTopPlay ? 'Cannot call out your own play' : gameState.topPlay ? `Call out ${topPlayer}` : 'Nothing to call out'} title={isSelfTopPlay ? 'Cannot call out your own play' : gameState.topPlay ? `Call out ${topPlayer}` : 'Nothing to call out'}><CircleHelp size={18} strokeWidth={2.5} /><span className="hidden sm:inline">Callout</span></button>
